...
<script>
  ...
  import { getDepartmentStoreDetails } from '../../api/api';
  ...
  export default {
    ...
    methods: {
      getMrpDetails(){
        ...
        getDepartmentStoreDetails(this.id).then(res =>{
          if(res.status===0){
            this.result=res.data.result;
            if(this.result.releaseType===101){
              this.result.releaseType='工服';
            }else if(this.result.releaseType===102){
              this.result.releaseType='百货';
            }else if(this.result.releaseType===103){
              this.result.releaseType='绿植';
            }else if(this.result.releaseType===104){
              this.result.releaseType='装饰用品';
            }
