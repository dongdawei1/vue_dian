<template>
  <div>
    <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
      <p>编辑简历</p>
      <div v-if="authentiCationFailure" class="authentiCationFailureClass">
        审核失败原因: {{this.ruleForm.authentiCationFailure}};
      </div>
...
<script>
  ...
  export default {
    data() {
      return {
        authentiCationFailure: false,//失败
        ...
        this.$refs['ruleForm'].validate((valid) => {
          if (valid) {
            create_resume(this.ruleForm).then(res => {
              ...
              if (res.status === 0) {
                this.$message.success("编辑成功，我们将尽快审核！");
        ...
        select_resume_by_id().then((res) => {
          if (res.status === 0) {
            if (res.data !== null) {
              ...
              if (this.ruleForm.authentiCationStatus === 3) {
                this.authentiCationFailure = true;
              }
              this.getRealName();
              this.loadAll();//获取完用户信息在调用
            } else {//没有创建过
              this.$message.error("先去创建简历吧！");
              this.$router.push({path: '/home/myRelease'});
              return false;
            }
          } else {
